...
module.exports = {
    ...
    $ConkittyEnvClass: EnvClass,

    $ConkittyGetEnv: function getEnv(obj) {
        return obj instanceof EnvClass ? obj : new EnvClass(obj instanceof Node ? obj : undefined);
    },

    $ConkittyClasses: function joinClasses() {
        var i, ret = [], arg;
    ...
    $ConkittyMod: function getModClass(name, val) {
        if (val) {
            return val === true ? name : name + '_' + val;
        }
    },

    $ConkittyChange: function getChangedClass(node, classes, remove) {
